 ...
 * implementations of the cc.lovezhy.stream.Stream interface and its primitive specializations.
 * Manages construction and evaluation of stream pipelines.
 *
 * <p>An {@code cc.lovezhy.stream.AbstractPipeline} represents an initial portion of a stream
 * pipeline, encapsulating a stream source and zero or more intermediate
 * operations.  The individual {@code cc.lovezhy.stream.AbstractPipeline} objects are often
 ...
 * {@code cc.lovezhy.stream.AbstractPipeline}, a shape-specific pipeline class which extends it
 * (e.g., {@code cc.lovezhy.stream.IntPipeline}) which is also abstract, and an operation-specific
 * concrete class which extends that.  {@code cc.lovezhy.stream.AbstractPipeline} contains most of
 ...
 * @param <S> type of the subclass implementing {@code cc.lovezhy.stream.BaseStream}
 * @since 1.8
 */
abstract class AbstractPipeline<E_IN, E_OUT, S extends BaseStream<E_OUT, S>>
    ...
    private Map<String, Map<?, ?>> prepareDataMap;

    public void setPrepareDataMap(String tag, Map<?, ?> singlePrepareDataMap) {
        prepareDataMap.put(tag, singlePrepareDataMap);
    }

    public Map<?, ?> getPrepareDataMap(String tag) {
        return prepareDataMap.get(tag);
    }
    ...
    AbstractPipeline(Supplier<? extends Spliterator<?>> source,
        ...
        // cc.lovezhy.stream.StreamOpFlag.combineOpFlags(sourceOrOpFlags, cc.lovezhy.stream.StreamOpFlag.INITIAL_OPS_VALUE);
    ...
    AbstractPipeline(AbstractPipeline<?, E_IN, ?> previousStage, int opFlags) {
        ...
        this.prepareDataMap = previousStage.prepareDataMap;
    }
     ...
     * @return a flat array-backed cc.lovezhy.stream.Node that holds the collected output elements
     */
    ...
    // cc.lovezhy.stream.PipelineHelper
    ...
    /**
     * Collect elements output from a pipeline into a cc.lovezhy.stream.Node that holds elements
     * of this shape.
     *
     ...
     * @return a cc.lovezhy.stream.Node holding the output of the pipeline
     */
    ...
    /**
     * Accepts a {@code cc.lovezhy.stream.Sink} which will receive the results of this operation,
     * and return a {@code cc.lovezhy.stream.Sink} which accepts elements of the input type of
     * this operation and which performs the operation, passing the results to
     * the provided {@code cc.lovezhy.stream.Sink}.
     *
     * @apiNote
     ...
     * the implementation for the {@code cc.lovezhy.stream.Stream#distinct()} method could just
     ...
     * {@code cc.lovezhy.stream.PipelineHelper} which describes the upstream intermediate
     ...
     * @param generator the array generator
     * @return a {@code cc.lovezhy.stream.Node} describing the result of the evaluation
     */
     ...
     * operation, using the specified {@code cc.lovezhy.stream.PipelineHelper} which describes the
