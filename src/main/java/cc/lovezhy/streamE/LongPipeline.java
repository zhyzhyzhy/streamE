...
abstract class LongPipeline<E_IN>
    ...
    /**
     * Adapt a {@code cc.lovezhy.stream.Sink<Long> to an {@code LongConsumer}, ideally simply
     * by casting.
     */
        ...
        if (sink instanceof LongConsumer) {
            ...
            if (Tripwire.ENABLED)
                Tripwire.trip(AbstractPipeline.class,
                              "using cc.lovezhy.stream.LongStream.adapt(cc.lovezhy.stream.Sink<Long> s)");
            return sink::accept;
        }
    }
    ...
    private static Spliterator.OfLong adapt(Spliterator<Long> s) {
        if (s instanceof Spliterator.OfLong) {
            ...
            if (Tripwire.ENABLED)
                Tripwire.trip(AbstractPipeline.class,
                              "using cc.lovezhy.stream.LongStream.adapt(Spliterator<Long> s)");
            throw new UnsupportedOperationException("cc.lovezhy.stream.LongStream.adapt(Spliterator<Long> s)");
        }
    }
    ...
    // Stateless intermediate ops from cc.lovezhy.stream.LongStream
    ...
    // Stateful intermediate ops from cc.lovezhy.stream.LongStream
    ...
    // Terminal ops from cc.lovezhy.stream.LongStream
    ...
    /**
     * Source stage of a cc.lovezhy.stream.LongPipeline.
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    static class Head<E_IN> extends LongPipeline<E_IN> {
        /**
         * Constructor for the source stage of a cc.lovezhy.stream.LongStream.
         *
    ...
    /** Base class for a stateless intermediate stage of a cc.lovezhy.stream.LongStream.
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    abstract static class StatelessOp<E_IN> extends LongPipeline<E_IN> {
        /**
         * Construct a new cc.lovezhy.stream.LongStream by appending a stateless intermediate
    ...
    /**
     * Base class for a stateful intermediate stage of a cc.lovezhy.stream.LongStream.
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    abstract static class StatefulOp<E_IN> extends LongPipeline<E_IN> {
        /**
         * Construct a new cc.lovezhy.stream.LongStream by appending a stateful intermediate
         * operation to an existing stream.
         * @param upstream The upstream pipeline stage
         * @param inputShape The stream shape for the upstream pipeline stage
         * @param opFlags Operation flags for the new stage
