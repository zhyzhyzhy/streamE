 ...
 * {@code double} types.  cc.lovezhy.stream.Streams differ from collections in several ways:
 *
 * <ul>
 ...
 *     but does not modify its source.  For example, filtering a {@code cc.lovezhy.stream.Stream}
 *     obtained from a collection produces a new {@code cc.lovezhy.stream.Stream} without the
 ...
 *     cc.lovezhy.stream.Stream operations are divided into intermediate ({@code cc.lovezhy.stream.Stream}-producing)
 ...
 * </ul>
 *
 * cc.lovezhy.stream.Streams can be obtained in a number of ways. Some examples include:
 * <ul>
 ...
 *     <li>cc.lovezhy.stream.Streams of file paths can be obtained from methods in {@link java.nio.file.Files};</li>
 *     <li>cc.lovezhy.stream.Streams of random numbers can be obtained from {@link java.util.Random#ints()};</li>
 ...
 *
 * <h2><a name="StreamOps">cc.lovezhy.stream.Stream operations and pipelines</a></h2>
 *
 * <p>cc.lovezhy.stream.Stream operations are divided into <em>intermediate</em> and
 ...
 * {@code cc.lovezhy.stream.Stream.filter} or {@code cc.lovezhy.stream.Stream.map}; and a terminal operation such
 * as {@code cc.lovezhy.stream.Stream.forEach} or {@code cc.lovezhy.stream.Stream.reduce}.
 *
 ...
 *
 * <p>Terminal operations, such as {@code cc.lovezhy.stream.Stream.forEach} or
 * {@code cc.lovezhy.stream.IntStream.sum}, may traverse the stream to produce a result or a
 ...
 * cc.lovezhy.stream.Streams facilitate parallel execution by reframing the computation as a pipeline of
 ...
 *
 * cc.lovezhy.stream.Streams enable you to execute possibly-parallel aggregate operations over a
 ...
 *     cc.lovezhy.stream.Stream<String> sl = l.stream();
 *     l.add("three");
 ...
 *
 * cc.lovezhy.stream.Stream pipeline results may be nondeterministic or incorrect if the behavioral
 ...
 * source (for example, {@code cc.lovezhy.stream.IntStream.range(0,5).parallel().map(x -> x*2).toArray()}
 ...
 *     List<String>results =
 *         stream.filter(s -> pattern.matcher(s).matches())
 *               .collect(cc.lovezhy.stream.Collectors.toList());  // No side-effects!
 * }</pre>
 *
 * <h3><a name="Ordering">Ordering</a></h3>
 *
 * <p>cc.lovezhy.stream.Streams may or may not have a defined <em>encounter order</em>.  Whether
 ...
 * ({@code cc.lovezhy.stream.Collectors.groupingBy()}) can be implemented more efficiently if ordering of elements
 ...
 * using a standard {@code cc.lovezhy.stream.Collector} as:
 * <pre>{@code
 *     List<String> strings = stream.map(Object::toString)
 *                                  .collect(cc.lovezhy.stream.Collectors.toList());
 * }</pre>
 *
 * <p>Packaging mutable reductions into a cc.lovezhy.stream.Collector has another advantage:
 ...
 *
 * <pre>{@code
 *     cc.lovezhy.stream.Collector<Employee, ?, Integer> summingSalaries
 ...
 *         = employees.stream().collect(cc.lovezhy.stream.Collectors.groupingBy(Employee::getDepartment,
 *                                                            summingSalaries));
 * }</pre>
 *
 ...
 *         = txns.parallelStream()
 *               .collect(cc.lovezhy.stream.Collectors.groupingBy(Transaction::getBuyer));
 * }</pre>
 ...
 * pipelines. cc.lovezhy.stream.Streams based on spliterators with the desired characteristics,
