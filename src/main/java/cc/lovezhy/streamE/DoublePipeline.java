...
abstract class DoublePipeline<E_IN>
    ...
    /**
     * Adapt a {@code cc.lovezhy.stream.Sink<Double> to a {@code DoubleConsumer}, ideally simply
     * by casting.
     */
        ...
        if (sink instanceof DoubleConsumer) {
            ...
            if (Tripwire.ENABLED)
                Tripwire.trip(AbstractPipeline.class,
                              "using cc.lovezhy.stream.DoubleStream.adapt(cc.lovezhy.stream.Sink<Double> s)");
            return sink::accept;
        }
    }
    ...
    private static Spliterator.OfDouble adapt(Spliterator<Double> s) {
        if (s instanceof Spliterator.OfDouble) {
            ...
            if (Tripwire.ENABLED)
                Tripwire.trip(AbstractPipeline.class,
                              "using cc.lovezhy.stream.DoubleStream.adapt(Spliterator<Double> s)");
            throw new UnsupportedOperationException("cc.lovezhy.stream.DoubleStream.adapt(Spliterator<Double> s)");
        }
    }
    ...
    // cc.lovezhy.stream.DoubleStream
    ...
    // Stateless intermediate ops from cc.lovezhy.stream.DoubleStream
            ...
            Sink<Double> opWrapSink(int flags, Sink<Double> sink) {
                return new Sink.ChainedDouble<Double>(sink) {
                    ...
                    public boolean cancellationRequested() {
                        ...
                        // pipeline is short-circuiting (see cc.lovezhy.stream.AbstractPipeline.copyInto).
    ...
    // Stateful intermediate ops from cc.lovezhy.stream.DoubleStream
    ...
    // Terminal ops from cc.lovezhy.stream.DoubleStream
    ...
    /**
     * Source stage of a cc.lovezhy.stream.DoubleStream
     *
     * @param <E_IN> type of elements in the upstream source
     */
    static class Head<E_IN> extends DoublePipeline<E_IN> {
        /**
         * Constructor for the source stage of a cc.lovezhy.stream.DoubleStream.
         *
    ...
    /**
     * Base class for a stateless intermediate stage of a cc.lovezhy.stream.DoubleStream.
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    abstract static class StatelessOp<E_IN> extends DoublePipeline<E_IN> {
        /**
         * Construct a new cc.lovezhy.stream.DoubleStream by appending a stateless intermediate
    ...
    /**
     * Base class for a stateful intermediate stage of a cc.lovezhy.stream.DoubleStream.
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    abstract static class StatefulOp<E_IN> extends DoublePipeline<E_IN> {
        /**
         * Construct a new cc.lovezhy.stream.DoubleStream by appending a stateful intermediate
         * operation to an existing stream.
         *
         * @param upstream the upstream pipeline stage
         * @param inputShape the stream shape for the upstream pipeline stage
