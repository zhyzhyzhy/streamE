...
abstract class IntPipeline<E_IN>
    ...
    /**
     * Adapt a {@code cc.lovezhy.stream.Sink<Integer> to an {@code IntConsumer}, ideally simply
     * by casting.
     */
        ...
        if (sink instanceof IntConsumer) {
            ...
            if (Tripwire.ENABLED)
                Tripwire.trip(AbstractPipeline.class,
                              "using cc.lovezhy.stream.IntStream.adapt(cc.lovezhy.stream.Sink<Integer> s)");
            return sink::accept;
        }
    }
    ...
    private static Spliterator.OfInt adapt(Spliterator<Integer> s) {
        if (s instanceof Spliterator.OfInt) {
            ...
            if (Tripwire.ENABLED)
                Tripwire.trip(AbstractPipeline.class,
                              "using cc.lovezhy.stream.IntStream.adapt(Spliterator<Integer> s)");
            throw new UnsupportedOperationException("cc.lovezhy.stream.IntStream.adapt(Spliterator<Integer> s)");
        }
    }
    ...
    // Stateless intermediate ops from cc.lovezhy.stream.IntStream
    ...
    // Stateful intermediate ops from cc.lovezhy.stream.IntStream
    ...
    // Terminal ops from cc.lovezhy.stream.IntStream
    ...
    /**
     * Source stage of an cc.lovezhy.stream.IntStream.
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    static class Head<E_IN> extends IntPipeline<E_IN> {
        /**
         * Constructor for the source stage of an cc.lovezhy.stream.IntStream.
         *
    ...
    /**
     * Base class for a stateless intermediate stage of an cc.lovezhy.stream.IntStream
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    abstract static class StatelessOp<E_IN> extends IntPipeline<E_IN> {
        /**
         * Construct a new cc.lovezhy.stream.IntStream by appending a stateless intermediate
    ...
    /**
     * Base class for a stateful intermediate stage of an cc.lovezhy.stream.IntStream.
     *
     * @param <E_IN> type of elements in the upstream source
     * @since 1.8
     */
    abstract static class StatefulOp<E_IN> extends IntPipeline<E_IN> {
        /**
         * Construct a new cc.lovezhy.stream.IntStream by appending a stateful intermediate
         * operation to an existing stream.
         * @param upstream The upstream pipeline stage
         * @param inputShape The stream shape for the upstream pipeline stage
         * @param opFlags Operation flags for the new stage
